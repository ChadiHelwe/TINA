...
from tina.collator.textual_entailment_collator import (
    AutoTextualEntailmentUnlikelihoodLossCollator,
)
from tina.dataset_dataloaders.textual_entailment_dataset_dataloader import (
    TextualEntailmentNegationAugmentedUnlikelihoodLossDataset,
)
from tina.loss import CrossEntropyAndUnlikelihoodLoss
from tina.model import AutoModelTE


def finetune_with_tina(
    ...
    weight_decay=0,
    num_labels=2,
    save_model_name="best_textual_entailment_negation_augmented_unlikelihood_loss_model",
    device="cpu",
    split=False,
):
    ...
    te_original_dataset = TextualEntailmentNegationAugmentedUnlikelihoodLossDataset(
        f"data/nli/{dataset_name}.csv", train_dataset_size
    )

    te_augmented_dataset = TextualEntailmentNegationAugmentedUnlikelihoodLossDataset(
    ...
    if "rte" in dataset_name:
        textual_entailment_collator = AutoTextualEntailmentUnlikelihoodLossCollator(
    ...
    criterion = CrossEntropyAndUnlikelihoodLoss()
    ...
    for epoch in range(0, epochs):
        ...
        for x, y, known_labels, unknown_labels in pbar:
            logits = model(x.to(device), y=y.to(device)).logits
            logits = logits.view(-1, num_labels)
            y = y.view(-1)
            loss = criterion(logits, y, known_labels, unknown_labels)
        ...
        if split:
            model.eval()
            with torch.no_grad():
                for x, y, known_labels, unknown_labels in tqdm(val_dataloader):
                    logits = model(x.to(device), y=y.to(device)).logits
                    logits = logits.view(-1, num_labels)
                    y = y.view(-1)
                    loss = criterion(logits, y, known_labels, unknown_labels)
